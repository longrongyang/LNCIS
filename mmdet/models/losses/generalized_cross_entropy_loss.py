...
class GeneralizedCrossEntropyLoss(nn.Module):
    ...
    def forward(self,
        ...
        label_one_hot = torch.nn.functional.one_hot(label, self.num_classes).to('cuda')

        loss_cls = ((1 - torch.pow(torch.sum(label_one_hot * cls_score, dim=-1), self.q)) / self.q).mean()
